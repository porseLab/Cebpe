...
```{r}
## sample statistics
## total: 120; bad: 31; extra: 19; analyzed: 70 (total-bad-extra)
...
```{r, eval=FALSE}
heatmap_lst <- list()
heatmap_lst[[1]] <- matrix2pheatmap(genes_cebpe[,c(8:9,11:26)], genes_cebpe$cluster, "row", colorRampPalette(c("dodgerblue4", "#2171b5", "grey97", "#fc8d59", "sienna3"), bias=1)(256))[[4]]
heatmap_lst[[2]] <- matrix2pheatmap(genes_cebpe[,c(28:29,31:46)], genes_cebpe$cluster, "row", colorRampPalette(c("dodgerblue4", "#2171b5", "grey97", "#fc8d59", "sienna3"), bias=1)(256))[[4]]
heatmap_lst[[3]] <- matrix2pheatmap(genes_cebpe[grep("Cebpa|Cebpe", genes_cebpe$gene),c(8:9,11:22)], c(1,1), "row", colorRampPalette(c("dodgerblue4", "#2171b5", "grey97", "#fc8d59", "sienna3"), bias=1)(256))[[4]]
heatmap_lst[[4]] <- matrix2pheatmap(genes_cebpe[grep("Cebpa|Cebpe", genes_cebpe$gene),c(28:29,31:42)], c(1,1), "row", colorRampPalette(c("dodgerblue4", "#2171b5", "grey97", "#fc8d59", "sienna3"), bias=1)(256))[[4]]
...
matrix2Heatmap(genes_cebpe[,c(8:9,11:26)], genes_cebpe$cluster, "row", colorRampPalette(c("dodgerblue4", "#2171b5", "grey97", "#fc8d59", "sienna3"), bias=1)(256))
dev.off()
...
pdf("/home/pundhir/project/chip-seq-analysis/analysis_cebpe/pdf/heatmap_gene_expression_refined.pdf", width=10)
  for(i in 1:3) {
    ...
    if(i==2 | i==1) {
      ...
      print(Heatmap(as.matrix(t(scale(t(mat_int[,c(8:9,11:22)])))), cluster_rows=F, cluster_columns=F,
...
p[[6]] <- ggplot((melt(summaryBy(myc_chip ~ cluster, data = genes_cebpe, FUN=list(median)))), aes(variable, cluster, fill= value)) + 
  ...
  scale_fill_gradientn(colours = (colorRampPalette(brewer.pal(9, col), bias=0.2)(256))) + theme_bw() +
  theme(legend.position="none", legend.text = element_text(size=6), axis.text.y = element_text(angle = -90, hjust = 1)) +
  coord_flip() + xlab("")
...
test <- test[which(test$cluster_deRefined!="F_rest_neutral"),]
...
df.melt <- melt(expr_data_norm_with_replicates[grep("^Myc$|^Cebpa$|^Cebpe$|^E2f4$|^Nfyb$|^Lin54$|^Rb1$|^E2f7$|^E2f8$", expr_data_norm_with_replicates$GeneID),
                               c("GeneID",
...
p2 <- ggboxplot(test[,c("myc_class", "log2FC_mm")], x="myc_class", y="log2FC_mm", notch=T, notchwdth=0.6, outlier.shape = NA, yscale="none", fill="myc_class") +
  xlab("") + ylab("Gene expr. FC (mm)") +
  scale_fill_manual(values=c("#8da0cb", "#66c2a5", "#fc8d62")) +
...
p3 <- ggboxplot(test[,c("myc_class", "myc_chip")], x="myc_class", y="myc_chip", notch=T, notchwdth=0.6, outlier.shape = NA, yscale="log2", fill="myc_class") +
...
cebp_granulopoiesis <- melt(expr_data_norm_with_replicates[grep("^Cebpa$|^Cebpe$|^E2f1$|^Cebpb$|^Myc$|^Mxd1$|^Max$",
...
cebp_monopoiesis <- melt(expr_data_norm_with_replicates[grep("^Cebpa$|^Cebpe$|^E2f1$|^Cebpb$|^Myc$|^Mxd1$|^Max$", 
                                                             expr_data_norm_with_replicates$GeneID),
                               c("GeneID",
                                 grep("lsk_.*wt", colnames(expr_data_norm_with_replicates), value = T),
                                 grep("pregm_.*wt", colnames(expr_data_norm_with_replicates), value = T),
...
df$cebp_classPeaksBroad <- sprintf("%s_%s", gsub(",cebpa", "", df$cebp_classPeaks), df$cebp_classPeaksLocation)
...
df$cebp_classPeaks <- factor(df$cebp_classPeaks, levels=c("cebpa", "cebpe,cebpa", "cebpe"), ordered = T)
...
p1 <- ggboxplot(df, x="cebp_classPeaks", y="dist_to_e2f1", notch=T, notchwdth=0.6, yscale="log2") + 
  stat_compare_means(comparisons = list(c("cebpe", "cebpe,cebpa"), c("cebpe", "cebpa")), method.args = list(alternative = "l"))
...
gXCebpAEPeaksGeneClass <- ggarrange(piePlot(melt(table(test[which(test$cebp_classPeaksBroad=="cebpa_enhancer"),c("cebp_classPeaksBroad", "cluster_de")])), title = "") + scale_fill_manual(values=c("#a6611a", "#dfc27d", "#7570b3", "#80cdc1", "#018571")),
          ...
          piePlot(melt(table(test[which(test$cebp_classPeaksBroad=="cebpe_promoter"),c("cebp_classPeaksBroad", "cluster_de")])), title = "") + scale_fill_manual(values=c("#a6611a", "#dfc27d", "#7570b3", "#80cdc1", "#018571")),
          piePlot(melt(table(test[which(test$cebp_classPeaksBroad=="cebpe_enhancer"),c("cebp_classPeaksBroad", "cluster_de")])), title = "") + scale_fill_manual(values=c("#a6611a", "#dfc27d", "#7570b3", "#80cdc1", "#018571")), nrow=2, ncol=2)
...
df$cebp_classPeaksBroad <- factor(df$cebp_classPeaksBroad, levels=c("cebpa_enhancer", "cebpa_promoter", "cebpe_enhancer", "cebpe_promoter"),
                                  ordered = T)
table(df$cebp_classPeaksBroad)
p1 <- ggboxplot(df, x="cebp_classPeaksBroad", y="cebpa_gmp_rep1", notch=T, notchwdth=0.6, yscale="log2") +
  stat_compare_means(comparisons = list(c("cebpa_enhancer", "cebpe_enhancer"), c("cebpa_promoter", "cebpe_promoter")), method.args = list(alternative = "g"))

p2 <- ggboxplot(df, x="cebp_classPeaksBroad", y="cebpe_lpmmy_rep1", notch=T, notchwdth=0.6, yscale="log2") +
  stat_compare_means(comparisons = list(c("cebpa_enhancer", "cebpe_enhancer"), c("cebpa_promoter", "cebpe_promoter")), method.args = list(alternative = "l"))
...
## Mmb9
mmb9 <- plot_gviz_track(GEN = "mm9", COOR = "chr2:164761367-164783160",
                       BAMFILE = browser_files[grep("cebp.*Rep1", browser_files$id),]$bam_files,
                       type = "a", maxlimit = 400,
                       BEDFILE=c("/home/pundhir/project/chip-seq-analysis/analysis_cebpe/analysis/E2f1_mm9_lifted.bed",
                                 "/home/pundhir/project/chip-seq-analysis/analysis_cebpe/analysis/cMyc_mm9_lifted.bed",
